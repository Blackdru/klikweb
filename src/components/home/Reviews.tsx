...
export default function Reviews() {
  ...
  const ReviewCard = ({ review }: { review: typeof reviews[0] }) => (
    <div className="bg-[#2D1543] rounded-2xl p-8 h-[420px] sm:h-[350px] flex flex-col justify-between">
  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-bl from-purple-900/40 via-black to-purple-900/40 text-white py-16 px-4">
            ...
            {Array.from({ length: windowWidth >= 768 ? Math.ceil(reviews.length / 2) : reviews.length }).map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-colors ${
                  currentSlide === index ? "bg-purple-500" : "bg-purple-500/20"
                }`}
